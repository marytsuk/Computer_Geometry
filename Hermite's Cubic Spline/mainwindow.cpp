...
void solve(double **mat)
{
    ...
    for(int i = 0; i < H; i++)
        S.push_back(rightMat[i]/mat[i][i]);
}
//Function computing value of r[i] at t
Point lineF(int i, double t)
{
    return (S[i]*(T[i+1] - t)*(T[i+1] - t)*(T[i+1] - t))/(6*(T[i+1]-T[i])) +
            (S[i+1]*(-T[i] +t)*(-T[i] +t)*(-T[i] +t))/(6*(T[i+1]-T[i])) +
            (P[i]/(T[i+1]-T[i]) - (S[i]*(T[i+1] - T[i]))/6)*(T[i+1]-t) +
            (P[i+1]/(T[i+1]-T[i]) - (S[i+1]*(T[i+1] - T[i]))/6)*(-T[i]+t);
}
...
void cubeSP()
{
        ...
        rightMat.push_back(Point(0, 0));
        for(int i = 1; i < size - 1; i++)
            rightMat.push_back(((P[i+1] - P[i])*6)/(T[i+1]-T[i]) - ((P[i]-P[i-1])*6)/(T[i] - T[i-1]));
        rightMat.push_back(Point(0, 0));
        ...
        {
            mat[i][i-1] = T[i]-T[i-1];
            mat[i][i] = 2*(T[i+1] - T[i-1]);
            mat[i][i+1] = T[i+1]- T[i];
        }

        if(W - 3 >= 0)
        {
            mat[H-2][W-3] = T[H-2]-T[H-3];
            mat[H-2][W-2] = 2*(T[H-1] - T[H-3]);
            mat[H-2][W-1] = T[H-1]-T[H-2];
        }
